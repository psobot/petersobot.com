...
def jekyllhtml2markdown s
  YAML::dump(YAML::load(s)) + "---\n" + ReverseMarkdown.new.parse_string(s.split('---')[-1].strip)
end

if ARGV[0]
  puts jekyllhtml2markdown open(ARGV[0]).read
else
  Dir.glob("source/_posts/*.html").each do |f|
    newname = f.split('.html')[-1]
    if newname[-1] == '-'
      newname = newname[0..-2] 
    end
    newname = newname + '.markdown' 
    open(newname, 'w').write jekyllhtml2markdown(open(f).read)
  end
end
